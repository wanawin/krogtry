# BUILD: core025_all3_member_straight_rank_streamlit_SINGLEFILE__2026-05-11_v8_STREAMKEY_FIX
...
BUILD_MARKER='BUILD: core025_all3_member_straight_rank_streamlit_SINGLEFILE__2026-05-11_v8_STREAMKEY_FIX'
...
def canonical_stream(state, game):
...
def normalize_history(df_raw):
    ...
    # Standard named-column format.
    if all(k in lower for k in ["date", "state", "game"]):
        date_col = lower["date"]
        state_col = lower["state"]
        game_col = lower["game"]
        result_col = None
        for cand in ["results", "result", "winning numbers", "winning_numbers", "winningnumbers", "number", "numbers", "draw"]:
            if cand in lower:
                result_col = lower[cand]
                break
        if result_col is None:
            # choose the first column after game with 4 extractable digits
            for c in df.columns:
                if c not in [date_col, state_col, game_col]:
                    sample = df[c].dropna().astype(str).head(25).apply(extract_pick4_digits)
                    if sample.str.len().eq(4).sum() >= max(1, len(sample)//3):
                        result_col = c
                        break
        if result_col is None:
            raise ValueError("History file loaded, but no usable Result/Results column was found.")

        out = df[[date_col, state_col, game_col, result_col]].copy()
        out.columns = ["Date", "State", "Game", "Results"]

    else:
        # Heuristic repair for headerless or oddly-named TXT exports.
        # Try to identify Date, State, Game, Result from columns.
        cols = list(df.columns)

        # If first row looks like header but pandas missed it, promote it.
        first_vals = [str(x).strip().lower() for x in df.iloc[0].tolist()] if len(df) else []
        if any(v in ["date", "state", "game", "result", "results"] for v in first_vals):
        ...
        for c in df.columns:
            ...
            if parsed.notna().sum() >= max(5, int(len(df) * 0.25)):
                date_col = c
                break

        result_col = None
        for c in df.columns[::-1]:
            vals = df[c].dropna().astype(str).head(100).apply(extract_pick4_digits)
            if vals.str.len().eq(4).sum() >= max(5, int(len(vals) * 0.25)):
                result_col = c
                break

        if date_col is not None and result_col is not None and len(df.columns) >= 4:
            # Pick state/game as two likely text columns between/near date and result.
            remaining = [c for c in df.columns if c not in [date_col, result_col]]
            text_cols = []
            for c in remaining:
                sample = df[c].dropna().astype(str).head(50)
                numeric_like = sample.str.fullmatch(r"\d+(\.\d+)?").fillna(False).mean() if len(sample) else 1
                if numeric_like < 0.8:
                    text_cols.append(c)
            if len(text_cols) >= 2:
                state_col, game_col = text_cols[0], text_cols[1]
            elif len(remaining) >= 2:
                state_col, game_col = remaining[0], remaining[1]
            else:
                raise ValueError("History file needs Date, State, Game, and Result columns.")

            out = df[[date_col, state_col, game_col, result_col]].copy()
            out.columns = ["Date", "State", "Game", "Results"]
        else:
            raise ValueError(
                "History file could not be normalized. It needs columns equivalent to Date, State, Game, Result/Results."
            )

    out["Date"] = pd.to_datetime(out["Date"], errors="coerce")
    out["Result"] = out["Results"].apply(extract_pick4_digits)

    # If result extractor failed because the Results column contains separated digits or extra text, retry row-wise.
    bad = out["Result"].astype(str).str.len().ne(4)
    if bad.any():
        out.loc[bad, "Result"] = out.loc[bad, "Results"].astype(str).apply(
            lambda x: "".join(re.findall(r"\d", x))[:4] if len(re.findall(r"\d", x)) >= 4 else ""
        )
    ...
    if out.empty:
        raise ValueError(
            "History loaded but normalized to 0 usable rows. Check that FULL HISTORY contains Date, State, Game, and 4-digit Results."
        )
...
st.info('Single-file v8 StreamKey fix. Smart-loads TXT/CSV/TSV and repairs common history formats before training.')
with st.sidebar:
    st.header('Inputs')
